...
- **Cron schedule firing confirmed (2026-08-11).** The `schedule` trigger fired on its own on the first real Tuesday (`gh run list --workflow=weekly_brief.yml` shows trigger `schedule`, ran 2026-08-11T14:09Z against a 13:00 UTC target, succeeded, committed a real `brief.md` update). GitHub Actions scheduling is fully verified end-to-end; no longer an open item.
...
- **Waivers + FAAB — Done (2026-08-11).**
  ...
  - **Ranking design:** rather than a separate VORP-style metric, each free agent is scored by the *exact* marginal points they'd add to your own optimal lineup right now -- `optimize_lineup(my_players + [candidate])` minus `optimize_lineup(my_players)`, reusing Phase 2's exact lineup optimizer unmodified. This single number already captures both raw value and roster need for free: a stud at a position you're already stacked at nets ~0 marginal value (correctly "not worth it"), while a modest player filling a genuinely weak/empty slot scores meaningfully higher.
  - **Degrades gracefully (PRD 6.3/R2):** when projections are unavailable, marginal value is meaningless (all zeros), so ranking falls back to roster need (dedicated positions you don't have enough rostered players to even fill) plus trending-add velocity (`GET /players/nfl/trending/add`, `fetch_trending_adds` in `repository.py` -- last-known-good cached, but treated as a non-load-bearing supplementary signal: a live failure with no cache is caught and the report proceeds without it, not an error).
  - **Waiver system detection:** `league.settings.waiver_type` -- **not documented in Sleeper's official API docs.** Mapping (`0` = Rolling, `1` = Reverse Standings, `2` = FAAB Bidding) was confirmed empirically by inspecting the actual dropdown in the sandbox league's League Settings UI (Chrome, logged in), matching its known-Rolling league against the observed value `0` and the dropdown's option order. Any other/missing value is treated as unknown and degrades to the priority-queue output path rather than guessing.
  - **FAAB path:** bid ranges via gap-based tiering on marginal value (same clustering approach as `vorp.build_tiers`), each tier mapped to a documented percentage-of-remaining-budget heuristic (`FAAB_BID_BANDS`) -- explicitly a heuristic, not a computed truth, since true market value depends on the whole league's bidding behavior, which this tool can't see. Budget pace tracking (`budget_pace_warning`) flags both overspending and over-conservatism by comparing fraction-of-budget-spent to fraction-of-season-elapsed.
  - **Rolling/reverse-standings path:** `priority_judgment` gives a hold-vs-spend call on the user's current waiver priority based on the top target's marginal value against a documented threshold.
  - `refresh` (R7) now appends the waiver summary as promised when it shipped in Phase 2.
  - Smoke-tested live against the sandbox league (empty rosters -- every free agent shows full raw value, which is the mathematically correct output for an empty roster) in both a real Rolling-Waivers run and a simulated FAAB run (settings overridden locally, since the sandbox league itself is Rolling). 23 new tests (`tests/test_waivers.py` + additions to `test_repository.py`/`test_cli.py`/`test_weekly_context.py`), 173 total passing.
  - Not yet verified: real bid amounts/pace against actual FAAB league behavior (no such league exists yet) and real marginal-value output against a non-empty roster -- both blocked on the same offseason transaction lock as the Phase 2 live-roster item above.

- **Trade Evaluation — Done (2026-08-11).**
  - `ffai/trade.py`, `ffai/cli.py trade --send NAME --receive NAME --with MANAGER` -- two-team trade evaluation. Players are named, not ID'd (`resolve_player_ids` does an exact case-insensitive match against the player directory and raises rather than guessing on zero or multiple matches -- PRD's "confidently wrong" anti-goal applies to picking the wrong player just as much as a bad number). Counterparty is named via their Sleeper display name (`roster.find_roster_by_display_name`, new), not a roster_id.
  - **Value design:** every player is looked up on a season-long VORP board (the same `vorp.py` machinery the draft assistant builds, fed season-aggregate projections instead of per-week). VORP alone is reported (`value_sent`/`value_received`/`net_vorp`) but is deliberately NOT the verdict -- the decisive number is `lineup_delta`: the change in each team's optimal lineup total (Phase 2's exact `optimize_lineup`, fed season totals) before vs. after the swap. A redundant addition at an already-stacked position can carry large VORP but ~zero `lineup_delta`; the disagreement between the two numbers IS the "accounts for roster construction/positional need" reasoning PRD 6.4 asks for, not a separate bolted-on heuristic. Confirmed in tests: a second elite RB behind an existing elite RB (only one RB slot) shows real VORP gain but exactly 0.0 lineup impact.
  - **Bye weeks:** flagged separately as a qualitative warning when 2+ rostered players at a multi-slot dedicated position would share a bye week post-trade (single-slot positions are exempt -- you'd only ever start one anyway, so a shared bye there is normal, not trade-specific).
